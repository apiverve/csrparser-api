...
class CsrparserValidationException implements Exception {
  final List<String> errors;

  CsrparserValidationException(this.errors);

  @override
  String toString() => 'CsrparserValidationException: ${errors.join("; ")}';
}
...
/// CSR Parser API client.
///
/// For more information, visit: https://apiverve.com/marketplace/csrparser?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [csr] (required) - The PEM-formatted Certificate Signing Request to parse
class CsrparserClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'csr': ValidationRule(type: 'string', required: true),
  };

  CsrparserClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/csrparser',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CsrparserValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CsrparserValidationException(errors);
    }
  }

  /// Execute a request to the CSR Parser API.
  ///
  /// Parameters are validated before sending the request.
  Future<CsrparserResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CsrparserException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CsrparserResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CsrparserException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CsrparserException('Resource not found');
      } else {
        throw CsrparserException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CsrparserException) rethrow;
      throw CsrparserException('Request failed: $e');
    }
  }
...
/// Exception thrown by the CSR Parser API client.
class CsrparserException implements Exception {
  final String message;

  CsrparserException(this.message);

  @override
  String toString() => 'CsrparserException: $message';
}
